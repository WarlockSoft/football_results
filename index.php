...
function match($team1, $team2){
	...
	$Team2 = getTeamSettings($team2);
		...
		/*Если одна команда сильнее другой более чем на одну единицу, то получаем коэфициент забитого гола, равного 
		 * Сила одной команды минус Сила другой команды (по модулю) деленная на 10. Это значение вычитаем из единицы,
		 * чтобы у команды с меньшим различием в Силе, коэфициент был относительно небольшой
		 * Если разница между силой команд равне или меньше единицы (по модулю), то коэфициент не учитывается.
		 */
		...
		//Проверяем, если первая команда сильнее второй
			if ($Team1["Strength"] > $Team2["Strength"]){
				 ...
				 * У второй команды коэфициент равен единице, а потому он не участвует в расчете. 
				 */
